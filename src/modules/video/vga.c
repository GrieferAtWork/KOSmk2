...
vga_setfont(MMIO void *regbase, struct vga_font const *__restrict font) {
 ...
 byte_t *src,*dst,*end; struct mman *omm;
...
PUBLIC void KCALL
vga_getpal(MMIO void *regbase, struct vga_pal *__restrict pal) {
 unsigned int i;
 byte_t *buffer = (byte_t *)&pal->vp_pal[0][0];
 vga_w(regbase,VGA_PEL_MSK,0xff);
 vga_w(regbase,VGA_PEL_IR,0x00);
...
PUBLIC struct vga_mode const vm_gfx = {
    .vm_att_mode          = 0x41,
    .vm_att_overscan      = 0x00,
    .vm_att_plane_enable  = 0x0f,
    .vm_att_pel           = 0x00,
...
#if 0
PUBLIC struct vga_mode const vm_modeX = {
    .vm_att_mode          = 0x41,
    .vm_att_overscan      = 0x00,
    .vm_att_plane_enable  = 0x0f,
    .vm_att_pel           = 0x00,
    ...
    .vm_seq_plane_write   = VGA_SR02_ALL_PLANES,
    .vm_seq_character_map = 0x00,
    .vm_seq_memory_mode   = 0x06, /* 0x0e */
};
#endif

PUBLIC struct vga_pal const vp_gfx_256 = {
...
PUBLIC struct vga_pal const vp_txt_64 = {
...
vga_ioctl(struct file *__restrict fp, int name, USER void *arg) {
 ...
 switch (name) {
 case VIO_SETMODE:
  if ((u16)(uintptr_t)arg >= VIO_MODE_COUNT) break;
  if ((u16)(uintptr_t)arg == vga->v_mode) return -EOK;
  if ((u16)(uintptr_t)arg == VIO_MODE_TEXT_COLOR_80X25) {
   vga_setmode(vga->v_mmio,&vm_txt);
   vga_setpal(vga->v_mmio,&vp_txt_64);
   vga_setfont(vga->v_mmio,vf_current);
  } else {
   vga_setmode(vga->v_mmio,&vm_gfx);
   vga_setpal(vga->v_mmio,&vp_gfx_256);
  }
