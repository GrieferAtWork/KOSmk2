...
GLOBAL_ASM(
L(RM_BEGIN                                        )
L(DEFINE_PRIVATE(rm_bios_int_begin)                  )
L(DEFINE_PRIVATE(rm_bios_int_num)                    )
L(DEFINE_PRIVATE(rm_bios_int)                        )
L(DEFINE_PRIVATE(rm_bios_int_end)                    )
L(rm_bios_int_begin:                              )
...
#define RM_SAFE_SEGMENTS
 __asm__ __volatile__(
     ...
     L(    pushq  %%rax                                )
     L(    lretq                                       )
     L(DEFINE_PRIVATE(rm_bios_int_exit)                   )
     L(rm_bios_int_exit:                               )
     L(    movabs $1f, %%rax                           )
     L(    jmp    *%%rax                               )
