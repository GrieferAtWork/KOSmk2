 ...
 */
#ifndef GUARD_INCLUDE_MODULES_LIBTERM_H
#define GUARD_INCLUDE_MODULES_LIBTERM_H 1
...
#define TERM_RGBA_CHANNEL_A 0
 struct PACKED { u8 a,b,g,r; };
 struct PACKED { s8 sa,sb,sg,sr; };
...
#define TERM_RGBA_CHANNEL_A 3
 struct PACKED { u8 r,g,b,a; };
 struct PACKED { s8 sr,sg,sb,sa; };
#else
#error FIXME
#endif
 u32 color;
...
struct term_operations {
    void (KCALL *to_putc)(struct term *__restrict term, char ch); /* [MANDATORY] Write a single character/byte. */
    void (KCALL *to_putb)(struct term *__restrict term, char ch); /* Write a a-z-style box character (NOTE: 'ch' is a lower-case ascii character). */
    void (KCALL *to_set_color)(struct term *__restrict term, struct term_rgba fg, struct term_rgba bg); /* Set the current text color. */
    void (KCALL *to_set_cursor)(struct term *__restrict term, struct point pos); /* Set the position of the cursor. */
    struct point (KCALL *to_get_cursor)(struct term *__restrict term); /* Returns the position of the cursor. */
#define TERM_SHOWCURSOR_NO  0
#define TERM_SHOWCURSOR_YES 1
    void (KCALL *to_show_cursor)(struct term *__restrict term, int cmd); /* Show/Hide the cursor. */
...
#define TERM_CLS_ALL    2 /*< Clear everything. */
    void (KCALL *to_cls)(struct term *__restrict term, int mode);
...
#define TERM_EL_ALL     2 /*< Erase the line. */
    void (KCALL *to_el)(struct term *__restrict term, int mode);
    ...
    void (KCALL *to_scroll)(struct term *__restrict term, offset_t offset);
    void (KCALL *to_set_title)(struct term *__restrict term, char *text); /*< Set the title of the terminal. */
    ...
    void (KCALL *to_putimg)(struct term *__restrict term, struct term_rgba image[]);
    struct point (KCALL *to_get_cell_size)(struct term *__restrict term); /* Returns the size of a cell in pixels. */
    void (KCALL *to_output)(struct term *__restrict term, char *text); /*< Output 'text' to the slave process ('write(amaster,text,strlen(text))'; amaster from <pty.h>:openpty) */
};
...
#define TERMINAL_FLAG_FREE  0x00000001 /*< Set if 'term_delete' should 'free(self)'. */
 ...
 ATOMIC_DATA uint32_t       tr_lock;    /*< Spinlock (Used internally). */
...
FUNDEF void KCALL term_fini(struct term *__restrict self);
FUNDEF void KCALL term_outc(struct term *__restrict self, char ch);
...
#endif /* !GUARD_INCLUDE_MODULES_LIBTERM_H */
