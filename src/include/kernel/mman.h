...
#define MREGION_INIT_WRITETHROUGH 0x80 /*< The region must track when parts have changed.
                                        *  When used with user-callbacks, `MREGION_INITFUN_MODE_SAVE' is signaled during synchronization. */
...
struct mregion {
 ...
 u16                            mr_gfunds; /*< [lock(mr_plock)][valid_if(MREGION_TYPE_ISGUARD(mr_type))] Guard funding.
                                            ...
                                            *   creation of a new region, the new region isn't created, and (if set)
                                            *   `mb_notify' of the associated branch is executed with `MNOTIFY_GUARD_END'.
 ...
 * WARNING: `mregion_setup' relies on the fact that the pointer passed
...
#define MMAN_SWAPMEM_NOSWAP GFP_NOSWAP /*< Don't initialize any kind of I/O during swap.
                                        ...
                                        *         from being synched+unloaded, while this flag is required
                                        *         to prevent any use of a potential swap partition. */
 ...
 * NOTE: This function must be called when `mman_mrestore_unlocked' is never called
