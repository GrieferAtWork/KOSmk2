...
mman_virtinfo(struct mman *__restrict mm,
 ...
 if (mm == THIS_MMAN) {
   ...
   * >>    ret
   * 
   * 'foo' is usually generated by the compiler to allow
   *  for position-independent executables, meaning that
   *  attempting to address 'bar' will instead yield the
   *  address of 'foo', which most likely doesn't have
   *  any debug information associated, and isn't what
   *  the caller wanted to learn about. */
  u8 temp[5];
  if (!copy_from_user(temp,addr,5) && temp[0] == 0xe9) {
   *(uintptr_t *)&addr += 5+*(s32 *)&temp[1];
  }
 }
#endif
 if (mm == &mman_kernel) {
