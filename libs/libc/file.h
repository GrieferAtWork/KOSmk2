...
#define FILE_BUFLOD  0x0001  /*< The current contents of 'f_buffer' have been loaded from 'f_fd',
                              *  starting at 'lseek(f_fd,0,SEEK_CUR)-(f_bufend-f_buffer)'. */
#define FILE_BUFCNG  0x0002  /*< [requires(FILE_BUFLOD)] The current contents of 'f_buffer' have changed since being loaded. */
/*                   0x07fc   */
...
#define FILE_BINARY  0x8000  /*< The file was opened in binary mode ('b'). */
 ...
 char             *f_buffer; /*< [lock(f_lock)][0..1][owned_if(!FILE_USERBUF)] Base address of the allocated buffer. */
 ATOMIC_DATA char *f_bufpos; /*< [lock(READ(f_lock))][0..1][>= f_buffer && <= f_bufend] Current buffer position. */
 char             *f_bufend; /*< [lock(f_lock)][0..1] End of the currently loaded (valid) buffer. */
 char             *f_bufmax; /*< [lock(f_lock)][0..1] End of the currently allocated buffer, or
